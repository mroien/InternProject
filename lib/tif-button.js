...
$(document).ready(function() {
		...
		jQuery(getRule(hostName)).after('<a href="'+getLink(hostName)+'"><input id="tif-button" type="button" /><div><canvas id="canvas" height="160px" width="180px"></canvas></div></a>');
		jQuery("#tif-button, #canvas").mouseenter(function() {
			// used to not make the mouseenter stutter when moved across fast
			
			$("#canvas").stop().slideDown(400)
				
		}).mouseleave(function() {
			
				$('#canvas').stop().slideUp(400)
