$(document).ready(function() {
	domainRules = [
		...
		},
		{
			...
			selector: '.tableitemQty, .familyItemQty',
	...
	jQuery(getRule(hostName)).after('<input id="tif-button" type="button" /><div class="chartTitle">Trackif Price Chart</div><a id="tif-chart-container" href="'+getLink(hostName)+'"><div class="tif-chart-wrapper"><canvas id="canvas" style="margin=15px"></canvas></div></a>');

	// jQuery to make the graph slide up and down on click
	$("#tif-button").click(function() {
		// used to not make the panel slide down
		$(".tif-chart-wrapper, .chartTitle").slideToggle(1);			
		});
/*
	// jQuery to make graph fadeto when the mouse leaves the tag
	$('.tif-chart-wrapper').mouseleave(function() {
		$('.tif-chart-wrapper').stop().fadeTo(1000, 0.7);
	});
	$('.tif-chart-wrapper').mouseenter(function () {
		$('.tif-chart-wrapper').stop().fadeTo(500, 1.0);
	});
*/
