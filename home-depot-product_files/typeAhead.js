...
(function(module, $) {
	...
	var urlRegex = /hd-(.+)\.homedepotdev\.com/,
		...
		template = {
			...
			category: [
				'<li>',
					'<a tabindex="{{tabindex}}" data-details="{{detailUrl}}" class="categories" data-category="suggested term" data-value="{{value}}" title="in {{termLong}}" href="{{url}}">',
						'{{{term}}} {{#desc}}<span>{{desc}}</span>{{/desc}}',
					'</a>',
				'</li>'].join(''),
			section: [
	...
	function keyboardNavigation(event) {
		...
		// Up, or shift + tab
			...
			event.preventDefault();
			navigateUp();
			//Get this item here to ensure we get the new active and not the previous active.
			var target = controls.terms.find('.active')[0];
			
			var currentSelectedText = $(target).attr('data-value');
		...
		if (isCallbackForCurrentTerm(data)) {
			...
			controls.terms.find('a').hoverIntent({
				...
				over: function(event) {
					...
					var target = controls.terms.find('.active')[0];					
					controls.searchInput.val($(target).attr('data-value'));
				},
	...
	*/
	function onSearchTermChanged(event, text) {
		...
		//If the triggering event is up arrow, down arrow, tab, or shift key, don't update the type ahead
		//term list.
		if(!(event.which === 38) && !(event.which === 9) && !(event.which === 40) && !(event.which === 16)){	
			...
			//console.log("Skipping update of term list");
		}
	}
	...
	function loadTerms(results) {
		...
		$.each(results, function (index, result) {
			...
			if("categories" in result){
				var fixedCategories = fixCategorySearchData(result.categories, result.termLong);
	...
	function fixCategorySearchData(results, currentParentTerm) {
		$.each(results, function (index, result) {
			// save the full term before truncating for title/alt tags
			result.termLong = result.term;

			result.value = currentParentTerm;
