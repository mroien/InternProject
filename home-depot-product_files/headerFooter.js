...
$(function(){
	...
	if(typeof $.fn.isotope === 'undefined'){ THDModuleLoader.$includeJS('/global/scripts/desktop/lib/plugins/jquery.isotope.min.js'); }
	//Include image load detection https://github.com/desandro/imagesloaded
	if(typeof $.fn.imagesLoaded === 'undefined'){ THDModuleLoader.$includeJS('/global/scripts/desktop/lib/plugins/jquery.imagesloaded.min.js'); }
...
function showGeoLink(){
	var isManualSetStore = function(){
			var thdPersist =  readCookie("THD_PERSIST"),
				...
				for(var i=0;i<thdPersistCookieArray.length-1;i++){
					...
					if(thdPersistCookieArray[i].indexOf('C27=') !== -1){
						  if(thdPersistCookieArray[i].indexOf('+') !== -1){
		...
		thirdparty = function(){
			 return (window.location.hostname.indexOf("www.") < 0 && window.location.hostname.indexOf("hd-") < 0  && window.location.hostname.indexOf("secure.") < 0 && window.location.hostname.indexOf("secure2.") < 0)? true : false;
		}(),
		isSetGeoLink = false;
		loggedUser = readCookie("THD_USERNAME"),
		geoCookie = getCookie("THD_GEOLOCALIZED"),
		isCartPage = document.getElementById('cartContainer'),
		isEmptyCartPage = document.getElementById('emptyCart'),
		...
		inStoreUser = "";
		if(isInstore === null && !(isThirdParty())){
		$.ajax({
				type: "GET",
                url: inStoreURL,
				async: false,
		...
		isSetGeoLink = (inStoreUser !== "" || loggedUser !== "" || isCartPage || isEmptyCartPage || isManualSetStore || geoCookie || isThirdParty()) ? false : true;
...
if($("#thdHeader").hasClass("pro")){
	...
	if (storeZip != '' && storeZip != '0') {
		if("geolocation" in navigator && showGeoLink()){
			...
			localizationPopupTrigger='<div class="locStoreBlock"><div class="fLeft"><p><span class="locYourStore">Your Store:</span> <a href="http://' + getHostNameNonSecure() + '/l/' + updateStoreDetails(storeName) + '/' + $.trim(storeLocDetails[1]) + '/'+ updateStoreDetails($.trim(storeLocDetails[0])) + '/' + $.trim(storeZip) + '/' + $.trim(storeNum) + '" class="locStoreName">' + storeString + '</a></p><p class="useCurrentLoc fLeft"><a href="#" class="useMyLocation"><i class="localizeIcon" ></i>Use Current Location</a> or <a class="thdOrange overlayTrigger" href="#localizationModalContent" rel="localizationModule" name="&amp;lpos=d-header-Change or Update Your Local Store" title="Change or update your local store" style="visibility: visible;">find store</a></p></div></div>';
		...
		if("geolocation" in navigator && showGeoLink()){
			...
			localizationPopupTrigger = '<div class="locStoreBlock"><div class="fLeft"><p><a class="thdOrange b overlayTrigger" rel="localizationModule" href="#localizationModalContent" title="Store Finder" name="&lpos=d-header-Select Your Local Store-localization off">Select </a> Your Local Store</p><p class="useCurrentLoc fLeft"><span  class="orText fLeft">or</span><a href="#" class="useMyLocation"><i class="localizeIcon"></i>Use Current Location</a></p></div></div>';
			THDModuleLoader.$includeJS('/static/scripts/modules/thdGeolocalize.js');
		}else{
			storeFinderUrl = '<a href="http://'+getHostNameNonSecure()+'/StoreFinder/index.jsp" name="&lpos=p-header-store finder">Store Finder</a></li>';
			localStoreCompare = '<a class="thdOrange b" href="' + storeFinderHref + '" title="Store Finder">Choose Your Local Store</a> <a class="tooltip" style="color:white;" title="In order to provide up to the minute updates on product pricing and availability at your local store, please select the store nearest to you." href="javascript:void(0);" title="Why?" id="why">(Why?)</a>';
			localizationPopupTrigger = '<a class="thdOrange b overlayTrigger" rel="localizationModule" href="#localizationModalContent" title="Store Finder" name="&lpos=p-header-Select Your Local Store-localization off"><i class="icon-localization-off"></i>Select </a> Your Local Store';
		}
		};
}
else{
	if (storeZip != '' && storeZip != '0') {
		...
		if("geolocation" in navigator && showGeoLink()){
			...
			storeFinderUrl = '<a href="http://'+getHostNameNonSecure()+'/StoreFinder/index.jsp" name="&lpos=d-header-store finder">Store Finder</a></li>';
			localStoreCompare = '<a class="thdOrange b" href="' + storeFinderHref + '" title="Store Finder">Choose Your Local Store</a> <a class="tooltip" style="color:white;" title="In order to provide up to the minute updates on product pricing and availability at your local store, please select the store nearest to you." href="javascript:void(0);" title="Why?" id="why">(Why?)</a>';
			localizationPopupTrigger = '<a class="thdOrange b overlayTrigger" rel="localizationModule" href="#localizationModalContent" title="Store Finder" name="&lpos=d-header-Select Your Local Store-localization off"><i class="icon-localization-off"></i>Select </a> Your Local Store';
		}
		}
	}
...
function updateCIinfo(){
	...
	if (setCIValue.indexOf("shopping-_-shopzilla") > -1) {
		createCICookie("CI_SHOPPING", "bizrate", 30, getPopupCurrentCookieDomain());
	}  
	if (setCIValue.indexOf("shopping-_-pricegrabber") > -1) {
		createCICookie("CI_SHOPPING", "pricegrabber", 30, getPopupCurrentCookieDomain());
	}                                                                                                                                                                                                                                                     
	if (setCIValue.indexOf("shopping-_-bingpa") > -1) {
		createCICookie("CI_SHOPPING", "bing", 30, getPopupCurrentCookieDomain());
	}
	if (setCIValue.indexOf("CJ-_-500871") > -1) {       
		createCICookie("CI_SHOPPING", "nexttag", 30, getPopupCurrentCookieDomain());
	}
	if (setCIValue.indexOf("CJ-_-500871") === -1 && setCIValue.indexOf("shopping-_-bingpa") === -1 && setCIValue.indexOf("shopping-_-pricegrabber") === -1 && setCIValue.indexOf("shopping-_-shopzilla") === -1) {
		if(setCIValue.indexOf("CJ-_-1319015") > -1 || setCIValue.indexOf("shopping") > -1) {
			createCICookie("CI_SHOPPING", "shopping", 30, getPopupCurrentCookieDomain());
		}
	}			
}
...
$(window).load(function() {
	...
	//Load the static fly
	if (THD.Global.HeaderFooter.hasOwnProperty("staticFlyClassic") && !THD.Global.HeaderFooter.staticFlyClassic) {
		THD.Global.StaticFly.init();
	}
...
.on("click","#quickorderModule, .quickorderModule, .quickOrder",function(e){
	...
	if(typeof(THD.MyAccount.signInModalValidation) === 'undefined'){
		var popupURL = document.location.protocol + "//" + document.location.hostname +"/static/myaccount/scripts/source/myAccount.popup.js"; // set the src attribute
		...
		*/
		...
		if(usercookie){
				...
				if(locStoreNumArr.length>0)	{
					...
					restoreLoclaization(locStoreNum,"http://www.homedepot.com");
...
function setCookieUserType(){
	     ...
	     var  usercookie=readCookie('THD_PERSIST'),
			 ...
			 if (usercookie){ 
					...
					restoreLoclaization(locStoreNum,path);
...
/* Data collector method for auto localization */
THD.Global.AutoLocalization.Promise.loaded.done(function () { 
	var autoLocCookie = getCookie("THD-LOC-STORE");
	
	_hddata.localStoreNum = autoLocCookie;
	if(typeof _T !== 'undefined'){ $(_T).trigger("autoLocalized"); }
})
...
!function (THD, $) {
	...
	var showFlyOut = function ($THDFlyout, $containerName){
        ...
        if ($FlyoutImages.length > 0) {
			...
			$FlyoutImages.imagesLoaded(function(){
				setupFlyoutGrid($FlyoutContainer, $Pod);
			});			
		} else {
			setupFlyoutGrid($FlyoutContainer, $Pod);
	...
	var hideFlyOut = function (THDFlyout){
		...
		$(this).find('.flyout.grid_24').replaceWith("<div class='flyout grid_24'></div>");
	};
...
//Gets called after DOM is ready
	$(document).on("click",'#veriSignImg',function(e){
		THD.log("Showing norton popup");
		var url = "https://seal.verisign.com/getseal?host_name="+window.location.host+"&amp;size=S&amp;use_flash=NO&amp;use_transparent=YES&amp;lang=en";
