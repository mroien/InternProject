...
function loadScript() {
  ...
  script.src = '//maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyDZQFBHW7S0-htE8AWCso4KjX9_72mVen8&sensor=false&' +
      'callback=initialize';
...
(function(){
	...
	function sortAndAppend(lat2, lon2) {
		...
		storearr = null;
		if (nearestStoreCount != "0")
		{
			$("#storePickUp").css('display','block');
			$("#iu-pickupInstoreWrap").css('display','block');
		}
	}
	function ShowStoreList()
	{
		...
		if (isDefault) {
			...
			$.ajax({
 				  url: 'http://geo.highspeedbackbone.net/api/IpLookup/resolve',
				  dataType: 'json'
				}).done(function(data) {
					geoLocationAvailable=true;
						lon2=data.data.longitude;
						lat2=data.data.latitude;
						var city =data.data.cityName ;
						var region = data.data.regionName; 
